...
## Base R Functions and survey package equivalents

| Base R | Survey Package | Notes | 
| :---: | :---: | :---: | 
| `mean()` | `svymean()` | Mean values (and proportions) |
|          | `svysciprop()` | Confidence intervals for proportions | 
| `sd()`   | `svysd()`   | Standard deviation (needs the `jtools` package) | 
| `sum()`  | `svytotal()` | Sums |
| `sum()/sum()` | `svyratio()` | Ratios |
| `lm()`, `glm()` | `svyglm()` | Regression | 
| `by()` | `svyby()` | Analysis in groups |
| `table()` | `svytable()` | Counts of observations in categories |
| `chisq.test()` | `svychisq()` | Chi-square test of association | 
| `hist()` | `svyhist()` | Histogram |
| `boxplot()` | `svyboxplot()` | Boxplot |
| `plot()` | `svyplot()` | Scatter plot |
| `barplot()` | `barplot()` | Bar plot |
