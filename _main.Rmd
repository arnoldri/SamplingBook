...
We can get hold of this value from a fitted `lm()` object using the `sigma()`
function:
```{r}
sigma(lm1)
```

The confidence interval anove is only valid if the model in equation \@ref(eq:simplelinearmodel) 
...
#| fig.align='center',
#| fig.width=5,
#| fig.cap="Linear Systematic Random Sample (LSRS).  Note that the sample is spaced out a regular intervals."
...
propotional to the number of enrolled students `Total`.)
```{r}
samp <- schools |> select.stsrs(n=120,  
                                stratumvar="Education_Region", 
                                allocation="equal", 
...
> The estimation of the **total** of a variable is only something
...
* If we're dealing with a **census**, then the Standard Error is zero, 
  ...
  isn't defined. (That's why it was `NaN` or `Inf` in our census examples.)
...
and assumes SRSWOR at both stages.  The `survey` package currently only allows for the analysis
...
<!--chapter:end:08-Calibration.Rmd-->
...
<!--chapter:end:10-DesigningSurveys.Rmd-->
...
<!--chapter:end:91-PlaceholderAppendix.Rmd-->
