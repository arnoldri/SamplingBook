...
as.data.frame(table(samp$Education_Region)) |>
  rename(Education_Region=Var1, SampleSize=Freq)
```
...
cluster sampling is **less efficient**: i.e. its standard errors are worse.  

We can see why when we consider **why** we take cluster samples: the reason is
predominantly one of controlling costs.  If we want to do a household survey
in a city like Wellington we could list all of the residential addresses, and
then draw a simple random sample of those dwellings.  That would mean a sample
...
if they have to visit houses multiple times to make contact with the residents.
...
they are a member of is selected.  So if we select clusters by SRSWOR unit $j$ 
in cluster $i$ has selection probability
\[
   \pi_{ij} = \frac{n}{N}
\]
if the clusters are selected by SRSWOR, leading to a weight of
\[
   w_{ij} = \frac{N}{n}
\]
where again note that $n$ is the number of clusters selected, and $N$
...
as.data.frame(table(samp$Territorial_Authority)) |>
  rename("Territorial Authority"=Var1, "NSchools"=Freq)
```
