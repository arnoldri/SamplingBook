...
\section{Sampling from discrete random variables}\label{sampling-from-discrete-random-variables}
...
trial is the same \(p\), and all the trials are independent. \(Y\) takes
values 0 to \(m\), and those probabilities can be calculated by
\[
   P(Y=k) = \binom{m}{k} p^k (1-p)^{m-k} = \frac{m!}{k!(m-k)!} p^k (1-p)^{m-k}
\]
R provides these probabilities in the function \texttt{dbinom()}

e.g.~\(m=10\) rolls of a die, and \(Y\) is the number of times we get a 6,
so \(p=1/6\)
...
If \(F(y)\) is the cdf of a random variable \(Y\) then we can draw a random
value of \(Y\) by drawing a random value \(U\) from a Uniform(0,1) distribution.

We then need to solve the equation
\[
   U = F(y)
\]
for \(y\). For many standard distributions (including the Normal and Gamma distributions)
...
\NormalTok{np }\OtherTok{\textless{}{-}} \DecValTok{101}
\NormalTok{yvec }\OtherTok{\textless{}{-}} \FunctionTok{seq}\NormalTok{(}\AttributeTok{from=}\DecValTok{0}\NormalTok{, }\AttributeTok{to=}\DecValTok{10}\NormalTok{, }\AttributeTok{length=}\NormalTok{np)}
\NormalTok{fvec }\OtherTok{\textless{}{-}}\NormalTok{ lambda}\SpecialCharTok{*}\FunctionTok{exp}\NormalTok{(}\SpecialCharTok{{-}}\NormalTok{lambda}\SpecialCharTok{*}\NormalTok{yvec)}
\FunctionTok{plot}\NormalTok{(yvec, fvec, }\AttributeTok{type=}\StringTok{"l"}\NormalTok{, }
     \AttributeTok{xlab=}\StringTok{"y"}\NormalTok{, }\AttributeTok{ylab=}\StringTok{"f(y)"}\NormalTok{, }\AttributeTok{main=}\StringTok{"Probability density of Exp(0.1)"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\caption{Exponential probability distribution}\label{fig:unnamed-chunk-33}
\end{figure}
...
\NormalTok{np }\OtherTok{\textless{}{-}} \DecValTok{101}
\NormalTok{yvec }\OtherTok{\textless{}{-}} \FunctionTok{seq}\NormalTok{(}\AttributeTok{from=}\DecValTok{0}\NormalTok{, }\AttributeTok{to=}\DecValTok{10}\NormalTok{, }\AttributeTok{length=}\NormalTok{np)}
\NormalTok{cdfvec }\OtherTok{\textless{}{-}} \DecValTok{1}\SpecialCharTok{{-}}\FunctionTok{exp}\NormalTok{(}\SpecialCharTok{{-}}\NormalTok{lambda}\SpecialCharTok{*}\NormalTok{yvec)}
\FunctionTok{plot}\NormalTok{(yvec, cdfvec, }\AttributeTok{type=}\StringTok{"l"}\NormalTok{, }
     \AttributeTok{xlab=}\StringTok{"y"}\NormalTok{, }\AttributeTok{ylab=}\StringTok{"f(y)"}\NormalTok{, }\AttributeTok{main=}\StringTok{"CDF of Exp(0.1)"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\caption{Exponential cumulative probability distribution}\label{fig:unnamed-chunk-34}
\end{figure}
...
\caption{10000 draws from an Exponential distribution}\label{fig:unnamed-chunk-35}
\end{figure}
